...
tf.flags.DEFINE_string("positive_data_file", "", "Data source for the real data")
tf.flags.DEFINE_string("negative_data_file", "", "Data source for the fake data")
...
tf.flags.DEFINE_string("x_raw", "", "The x raw that will be evaluated")
...
with open(FLAGS.trigram_dir+"all_top_n_neurons.txt", 'wb') as f:
    pickle.dump(all_top_n_neurons, f)

np.save(FLAGS.trigram_dir+"weights",weights)

np.save(FLAGS.trigram_dir+"all_wi_ai", all_wi_ai)
